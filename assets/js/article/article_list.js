$(function () {
    ...
    $('#form-search').on('submit', function (e) {
        ...
        if (cate_id != null && cate_id != undefined && cate_id !== '') {
            q.cate_id = cate_id
        }
    ...
    function renderPage(total) {
        ...
        laypage.render({
            ...
            jump: function(obj, first){
                //可以通过first的值,来判断是通过哪种方式触发jump回调
                //如果first的值是turn,证明是方式2触发的
                //否则就是方式1,触发的
                console.log(first);
        ...
        layer.confirm('确定删除', {icon: 3, title:'提示'}, function(index){
            $.ajax({
                ...
                success: function(res){
                    ...
                    layer.msg('删除文章成功')
                    //当数据删除完成后,需要判断当前这一页中是否有剩余的数据,如果没有剩余数据了,则让页码值-1之后,再调用initTable方法
                    //4
                    if(len === 1){
